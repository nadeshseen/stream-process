...
char * get_stream_attributes(char *query);
char* substr(const char *src, int m, int n);
void lowr(char *arg);
static void custom_stream_parse_function(char *query_string);
...
exec_simple_query(const char *query_string)
{
	...
	foreach(parsetree_item, parsetree_list)
	{
		...
		int16		format;
//		char *str = lfirst_node(char, parsetree_item);
//		printf("%d \n", parsetree->stmt_location);
		...
		querytree_list = pg_analyze_and_rewrite(parsetree, query_string,
												NULL, 0, NULL);
//		printf("Address: %p \n", querytree_list->elements->ptr_value);
			...
			case 'Q':			/* simple query */
				{
					...
					if (am_walsender)
					{
						 ...
						 * Also, we insert the rows, and columns on which the aggregate is performed
						 * and initialise them.
						 */
						if(!created){
							char tmp_query[300];
						...
						/*
						 * Allocate space to check if query is a stream
						 */
						char *substr = (char *)malloc(7);
						lowr(query_string);
						strncpy(substr, query_string, 7);
...
char * get_stream_attributes(char *query) {
	...
	while (token != NULL)
	{
		...
		if(cnt==2){
			...
			stream_attrs.relation_name = strdup(tname);
		}
    ...
    while(origquery[i] != '\0')
    {
        if(origquery[i] != ' ')
        {
        	nospacequery[j++] = origquery[i];
        }
        i++;
    }
    nospacequery[j] = '\0';

    int stream_attr_index = 2;
    if(hasAttr==0){
		char *b;
		char *e;
		int indexb;
		...
		b = strchr(nospacequery, '(');
		indexb = (int)(b - nospacequery);
		e = strchr(nospacequery, ')');
		indexe = (int)(e - nospacequery);


		char* attrvals = substr(nospacequery, indexb+1, indexe);


		char *valtok = strtok(attrvals, ",");
		int attrcnt = 0;
		while (valtok != NULL)
		{
			if(attrcnt==stream_attr_index){
				stream_attrs.str = strdup(valtok);
			}
			attrcnt++;
			valtok = strtok(NULL, ",");
		}
		...
		char* attrnames = substr(nospacequery, indexb+1, indexe);


		char *valtok = strtok(attrnames, ",");
		char *splitatrrs[3];

		int attrcnt = 0;
		while (valtok != NULL)
		{
			splitatrrs[attrcnt] = valtok;
			lowr(valtok);
			if(strcmp(valtok,"marks")==0){
				stream_attr_index = attrcnt;
				stream_attrs.attr_name = strdup("marks");
			}
			attrcnt++;
			valtok = strtok(NULL, ",");
		}
		...
		//attr values
		b = strrchr(nospacequery, '(');
		indexb = (int)(b - nospacequery);
		e = strrchr(nospacequery, ')');
		indexe = (int)(e - nospacequery);

		char* attrvals = substr(nospacequery, indexb+1, indexe);

		char *valtok2 = strtok(attrvals, ",");
		char *splitvals[3];
		attrcnt = 0;
		while (valtok2 != NULL)
		{
			splitvals[attrcnt] = valtok2;
			attrcnt++;
			valtok2 = strtok(NULL, ",");
		}

		stream_attrs.str = strdup(splitvals[stream_attr_index]);
...
/*
 * This function uses the existing parser capabilities to retrieve the column names
 * and values of the stream insert, to compute the aggregate and store it in-memory.
 * This function sets the stream_values struct.
 */
static void custom_stream_parse_function(char *query_string){

	MemoryContext oldcontext;
	ListCell   *parsetree_item;
	ListCell   *target_entry;
	ListCell   *range_table_entry;
	ListCell   *value_entry;
	ListCell   *lc;
	oldcontext = MemoryContextSwitchTo(MessageContext);
	...
	foreach(parsetree_item, parsetree_list)
	{
		...
		start_xact_command();
		oldcontext = MemoryContextSwitchTo(MessageContext);
		Query *query = parse_analyze(parsetree, query_string, NULL, 0,
								  NULL);

		List *target_list = query->targetList;
		List *rtable = query->rtable;
		Node *parseTree = parsetree->stmt;
		InsertStmt *stmt = (InsertStmt *) parseTree;
		SelectStmt *selectStmt = (SelectStmt *) stmt->selectStmt;
		List	   *valuesLists = selectStmt->valuesLists;
		List *exprlist = (List *) linitial(valuesLists);
		bool id_first = false;
		int i =0;

		foreach(target_entry, target_list){
			TargetEntry *te = (TargetEntry *) lfirst(target_entry);
			if(strcmp(te->resname, "id") == 0){
				if(i == 0){
					id_first = true;
				}
				else{
		...
		// Iterating through the expressions list and retrieving the values.
		int j = 0;
		foreach(lc, exprlist)
		{
			Node	   *e = (Node *) lfirst(lc);
			A_Const    *con = (A_Const *) e;
			Value	   *val = &con->val;
			if(val->type == T_Integer){
				if(j==0 && id_first){
					stream_values.id = val->val.ival;
				}
				else if (j == 0 && !id_first){
					stream_values.marks = val->val.ival;
				}
				else if(j ==1 && id_first){
					stream_values.marks = val->val.ival;
				}
				else if(j ==1 && !id_first){
					stream_values.id = val->val.ival;
